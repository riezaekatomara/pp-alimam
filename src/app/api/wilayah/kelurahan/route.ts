...
// Cache per kecamatan
const kelurahanCache: Map<string, { data: any[]; timestamp: number }> = new Map();
...
export async function GET(request: Request) {
  try {
    ...
    // Check cache
    const cached = kelurahanCache.get(kecamatanId);
    ...
    // Cache the data
    kelurahanCache.set(kecamatanId, { data, timestamp: Date.now() });

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error("Error fetching kelurahan:", error);
    return NextResponse.json(
      { success: false, error: "Gagal mengambil data kelurahan" },
      { status: 500 }
    );
  }
}
